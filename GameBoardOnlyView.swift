...
struct GameBoardOnlyView: View {
    ...
    var body: some View {
        ...
        .gesture(
            SpatialTapGesture()
                .onEnded { _ in
                    if gameManager.currentPiece != nil && !gameManager.isPaused && !gameManager.isGameOver {
                        let now = Date()
        ...
        if let piece = gameManager.currentPiece {
            for (i, row) in piece.blocks.enumerated() {
                for (j, cell) in row.enumerated() where cell == 1 {
                    let block = createBlock(shape: piece.shape, size: blockSize, isInteractive: true)
                    let gridX = piece.x + j
                    let gridY = piece.y + i
    ...
    func createBlock(shape: TetrisShape, size: Float, isInteractive: Bool) -> ModelEntity {
        ...
        if isInteractive {
            ...
            block.components.set(CollisionComponent(shapes: [.generateBox(size: [size * 0.88, size * 0.88, size * 0.88])]))
